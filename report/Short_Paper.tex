...
\subsection{Spatial Cold Spots, Deserts, and Typology}
Local Moran's $I$ (Fig.~\ref{fig:lisa}) resolves the clustering into large,
statistically significant low-low ``cold spots'' on each city's periphery and
high-high cores, identifying contiguous under-served areas rather than isolated
tracts. Because a mobility desert is defined \emph{within} each city (bottom MES
quartile), the $\approx 25\%$ share is definitional; the substantive results are
that desert tracts are higher-income peripheries that nonetheless contain large
minority populations (52--64\% non-white in Chicago/Houston), that the
classification is stable under threshold changes (Jaccard $0.80$--$0.83$ across
the 20th/25th/30th percentiles), and that compounded deserts (low on $\geq 2$
dimensions) concentrate the disadvantage. K-means recovers interpretable
profiles---a well-served multimodal core, a uniformly under-served periphery,
and, in Houston, a distinctive ``walk-rich, bike-poor'' class (537 tracts);
silhouettes for the selected $k$ are $0.41$--$0.48$ and the partitions are
reproducible across initializations (mean ARI $=1.00$). Because $k$ differs by
city, similarly named profiles are compared by centroid shape, not label.
...
\caption{Local Moran's $I$ (LISA) clusters of the MES (Chicago, Houston,
Seattle). Red = high-high (well-served cores); blue = low-low (under-served
cold spots).}
\label{fig:lisa}
\end{figure*}
...
income association is negligible except in Seattle, confirming that ordinary
significance was overstated. Dropping the OSM-sourced EV layer leaves the
findings intact on every axis---the income, renter, and zero-vehicle
correlations move by $|\Delta r|\leq 0.05$, the desert set overlaps by a Jaccard
of $0.71$--$0.92$, and spatial clustering is essentially unchanged---so the
conclusions do not depend on the least-certain dimension.
...
policy value. In practice this supports neighborhood-level targeting: ranking
tracts, separating single- from multi-dimension deficits, and suggesting
differentiated interventions such as cycling investment for walk-rich/bike-poor
areas and first/last-mile transit for under-served peripheries.

\emph{Cross-city reading:} because normalization is performed within each city,
absolute MES levels are not comparable across cities---a score of 70 in Houston
is not equivalent to 70 in Chicago. The robust comparative result is instead
that the same qualitative pattern (strong spatial clustering, positive
car-free/renter associations, and an income gradient largely explained by urban
form) recurs in all three metros, with Seattle the one case where a negative
income association survives controls and therefore merits city-specific study.
...
service-quality data, and---most directly---develops a \emph{demand-adjusted MES}
that combines infrastructure supply with transportation need so the supply--need
relationship can be modeled rather than inferred. The full pipeline and dashboard
are open-source, making the method transferable to any city with open data.
