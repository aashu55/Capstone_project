...
into a single tract-level composite (MES) that is internally consistent
(Cronbach-style inter-index agreement reported) and stable under alternative
weighting schemes (sign and significance of downstream correlations unchanged
across equal, entropy, PCA, and AHP weights)?
...
tracts are under-served, but neither places transit alongside other modes.
...
\]
Spearman rank correlation was checked as a monotonic robustness variant and
agreed in sign and significance.
...
adding transit \emph{service quality} (headway reliability); incorporating
demand-side accessibility (jobs reachable within a travel-time budget) to test
the supply--demand mismatch directly; and a longitudinal extension to measure
whether targeted investment narrows the gaps the MES identifies.
