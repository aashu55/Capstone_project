...
mismatch rather than a simple ``poor areas are under-served'' narrative (§5).
...
§4.7, where it largely disappears once urban form is controlled. All coefficients
are associational, not causal, and (as §4.7 shows) their significance is
...
Because the bivariate correlations (§4.2) neither control for confounders nor
account for the strong spatial dependence (§4.3), four robustness checks were
...
layer barely changes its correlation with income ($|\Delta r| = 0.002$--$0.053$),
so the conclusions are not driven by the least-certain dimension.
...
\toprule
\textbf{City} & \textbf{MES--income, bivariate} & \textbf{+ urban-form controls} & \textbf{Spatial (AIC)} & \textbf{Cluster ARI} & \textbf{EV $|\Delta r|$} \\
\midrule
...
ordinary significance is corrected via spatial regression, §4.7). (3) The
...
therefore report spatial-lag/error models and urban-form controls (§4.7), which
