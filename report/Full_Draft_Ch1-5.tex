...
(Moran's $I\approx0.9$), observations are not independent, so the correlation
$p$-values are optimistic; a spatial-regression (spatial-error or lag) model is
the appropriate next step. The tract-level associations are also ecological and
do not imply individual-level relationships.
