...
\lhead{\small EMGT 7945 --- Full Draft (Ch. 1--5)}
\rfoot{\small\thepage}
...
\thispagestyle{empty}
\begin{center}
{\large\bfseries MS PROJECT REPORT --- FULL DRAFT (CHAPTERS 1--5)}\\[2pt]
...
significant spatial clustering (global Moran's $I = 0.82$--$0.92$, $p < 0.001$)
and is \emph{negatively} associated with median household income and
\emph{positively} associated with the share of zero-vehicle and renter
households --- indicating that supply-side mobility infrastructure concentrates
in dense, lower-income urban cores, so that mobility ``deserts'' fall
disproportionately in higher-income, auto-oriented peripheries. This inverts the
naive expectation and motivates a careful supply-versus-demand interpretation,
which is the study's central contribution. The methodology is released as an
open-source pipeline and an interactive dashboard so any city with open data can
...
into a single tract-level composite (MES) that shows coherent internal structure
(inter-index correlations and Cronbach's $\alpha$ reported) and is stable under
alternative weighting schemes (sign and significance of downstream correlations
unchanged across equal, entropy, PCA, and AHP weights)?
...
tract-level index, where prior tools measure one dimension or one scale (§2.7).
...
supply-side mobility infrastructure concentrates in dense, lower-income,
higher-renter/car-free cores; consequently the MES is \emph{negatively}
correlated with income and mobility deserts fall in higher-income peripheries.
This reframes the equity question around a supply--demand mismatch rather than a
simple ``poor areas are under-served'' narrative (§5).
...
critique as a design requirement (§3.6--§3.7) rather than an afterthought,
...
validation~\cite{rousseeuw1987} for typology discovery. These are chosen (§3.9--
§3.11) because they directly answer the distributional and structural research
...
--- each must be an accepted proxy for its dimension in the literature (§2);
...
enough for within-city comparison (revisited as a limitation in §5.4).
...
Walkability Index per tract (§3.4). \textbf{Bicycle.} From the OpenStreetMap
...
\emph{levels} are therefore interpreted with caution (§5.4), while within-city
...
tested against three alternatives (§4.6): \emph{entropy} weights (from each
sub-index's dispersion), \emph{PCA} weights (first-component loadings), and
\emph{AHP} weights (expert pairwise judgment: transit 0.40, walk 0.30, bike 0.15,
EV 0.15). If the sign, magnitude, and significance of the equity correlations are
...
(reported in §4.2); it agrees in sign with Pearson across all cities.
...
right-skewed (means 5--7; most tracts near zero with a few high-provision
...
Seattle & 495   & 36.2 & 14.1 & 18.9 & 33.5 & 63.8 & 7.3 & 40.3 \\
\bottomrule
\end{tabularx}
\end{table}
...
non-white is positive but weak ($r=0.10$ to $0.14$). In short, supply-side
mobility infrastructure is greatest where residents are lower-income, car-free,
and renting --- the dense urban core --- and lowest in higher-income
auto-oriented peripheries. Figure~\ref{fig:corr} visualizes the full correlation
structure for Chicago. A Spearman rank check agrees in sign with every Pearson
...
non-redundant internal structure (mean inter-index Pearson $r = 0.45$--$0.62$;
Cronbach's $\alpha = 0.74$--$0.85$), confirming they co-vary spatially yet each
contributes distinct information (RQ1).
...
that cycling is the systematically weakest dimension.
...
Re-computing the MES under entropy, PCA, and AHP weights leaves the equity
conclusions unchanged in sign and significance (Chicago shown): the MES--renter
correlation stays $r\approx0.57$--$0.59$ and MES--income stays negative
($-0.085$ to $-0.145$) across all four schemes. The entropy scheme up-weights
bicycle (0.68) because that sub-index has the highest dispersion, yet even this
extreme reweighting does not flip any sign --- satisfying the RQ1 stability
criterion.
...
Three findings are robust across all three cities. First, mobility supply is
\emph{highly spatially clustered} (Moran's $I=0.82$--$0.92$): inequity is
structural and place-based. Second, the MES is \emph{negatively} associated with
income and \emph{positively} with car-free and renter shares: supply concentrates
in dense, lower-income cores. Third, the resulting mobility deserts are therefore
predominantly \emph{higher-income, auto-oriented peripheries}, though they still
contain substantial minority populations (52--64\% non-white in Chicago/Houston).
Bicycle infrastructure is the weakest dimension everywhere, and its scarcity
defines a recurring ``walk-rich, bike-poor'' neighborhood type.

\subsection{Interpretation: A Supply--Demand Reframing}
The negative MES--income relationship inverts the naive ``poor neighborhoods are
under-served'' expectation, but it is not a claim that low-income residents are
well served in every sense. It is a \emph{supply-side} result: infrastructure
follows density and land-use mix, which correlate with lower incomes and higher
renter/car-free shares in these three cities. The equity implication is a
\emph{mismatch} framing. Where dense low-income cores are well supplied, the
concern shifts to service \emph{quality} and \emph{affordability} (not captured
by supply counts); where higher-income peripheries score low, their residents are
largely car-dependent by means, so low supply is less acute --- \emph{except} for
the car-free and minority households embedded in those peripheries, who face the
sharpest supply--demand gap. Compounded deserts and LISA cold spots identify
precisely those tracts, which is where the framework adds the most policy value.
...
Four limitations bound the conclusions. (1) \emph{Supply-side only}: the MES
...
snapshot for three metros and is not yet generalizable nationally.
...
alongside the demographic cross-tabs; incorporating demand-side accessibility
(jobs reachable within a travel-time budget) to test the supply--demand mismatch
directly; and a longitudinal extension to measure whether targeted investment
narrows the gaps the MES identifies.
...
dense, lower-income cores --- reframing the equity question as a supply--demand
